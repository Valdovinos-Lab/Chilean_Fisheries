function dXdt = atn_ode_biomass_catch_gainfish_gain_loss_fast_H50(~,X,Data)
% ----------------------------------------------------------------------- %
% ----------------------------- PARAMETERS ------------------------------ %
% ----------------------------------------------------------------------- %
...
B = X(1:GuildInfo.nGuilds); %B = Binit(1:GuildInfo.nGuilds) 
% ----------------------------------------------------------------------- %
...
ProducerFisheries = ((0.125*bProducerGuilds)/100).*ProducerHarvest;
%ConsumerFisheries = ((10*bConsumerGuilds)/100).*ConsumerHarvest;

%calculating the removal to harvested species 
%all consumers without filters and hervibores
Hconsumers = ((10*bConsumerGuilds([1,4,7,10,12:18,23],:))/100);
%all filters and hervibores
HfilandHerb = ((23*bConsumerGuilds([29,34,46],:))/100);
%assigning the removal to harvested species 
ConsumerFisheries  = zeros(length(GuildInfo.iConsumerGuilds),1);
ConsumerFisheries([1,4,7,10,12:18,23],:) = Hconsumers;
ConsumerFisheries([29,34,46],:) = HfilandHerb;
...
plankton = dBdt(95,:);
plankton = plankton + ((61291*12)/100);
dBdt(95,:) = plankton;
